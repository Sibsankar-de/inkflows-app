...
export const HeaderNav = () => {
    ...
    return (
        <>
                ...
                <section className='if-nav-sec'>
                    <div>
                        <Link to='/home' className='if-url-normal'>
                            <button className='if-btn if-nav-btn' data-tooltip-id='if-nav-tooltip' data-tooltip-content={'Home'}>
                                <span><i className="ri-home-2-line fs-4"></i></span>
                            </button>
                        </Link>
                    </div>
                    <div className='if-nav-search-box'>
                        <span className='if-search-logo-box'><i className='ri-search-2-line'></i></span>
                        <input type="text" placeholder='search blogs' className='if-search-bar' onFocus={(e) => { setOpenSearchBox(true) }} onClick={e => e.stopPropagation()} />
                    </div>
                </section>
            ...
            <SearchBox openState={openSearchBox} onClose={() => setOpenSearchBox(false)} />
        </>
    )
}
...
const ProfileDropdown = ({ activeState, onClose, currentUser }) => {
    ...
    return (
        <SelectDropdown activeState={activeState} className='if-profile-dropdown-box'>
            ...
            <PreAuthContainer>
                <div className='if-prof-dropdown-sections'>
                    <button className="if-btn-2 if-btn-blue--grad justify-content-center" onClick={() => navigate('/auth/login')}><span><i class="ri-loop-right-line"></i></span><span>Sign in</span></button>
                </div>
            </PreAuthContainer>
            <div className='if-prof-dropdown-sections'>
            ...
            <div className='d-grid gap-1'>
                <div>
                    <a href="" className='if-url-coloured'><span><i className="ri-question-line"></i></span> <span>How to write a blog?</span></a>
                </div>
                <div>
                    <a href="" className='if-url-coloured'><span><i className="ri-key-2-line"></i></span> <span>Privacy terms</span></a>
                </div>
            </div>
        </SelectDropdown>
    )
